...
export default class extends Controller {
  ...
  updateOptionsForViewMode(viewMode) {
    ...
    if (this.hasGroupingIconTarget) {
      ...
      if (viewMode === "board") {
        // Columns icon
        icon.innerHTML =
          '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 4H5a2 2 0 00-2 2v4m6-6h6m-6 0v6m6-6v6m6-6a2 2 0 012 2v4M4 10v6a2 2 0 002 2h4m-6-8h6m0 8v6m0-6h6m0 6v6m0-6a2 2 0 012-2h4"></path>';
      } else {
        // List/rows icon
        icon.innerHTML =
  ...
  updateToggleVisual(param, checked) {
    ...
    const toggleContainer = this.element.querySelector(
      `[data-display-options-target="${param.replace(/_([a-z])/g, (m, p1) =>
        p1.toUpperCase()
      )}Toggle"]`
    );
  ...
  savePreferences() {
    ...
    // Add current checked properties
    const container = this.element.querySelector('[data-display-options-target="panel"]');
    const checkedBoxes = container.querySelectorAll('input[type="checkbox"]:checked');
    const properties = Array.from(checkedBoxes).map((cb) => cb.value);
  ...
  getCurrentFormValues() {
    ...
    const container = this.element.querySelector('[data-display-options-target="panel"]');
    const checkedBoxes = container.querySelectorAll('input[type="checkbox"]:checked');
    const properties = Array.from(checkedBoxes)
      .map((cb) => cb.value)
      .sort()
      .join(",");
