...
class Settings::TimeZoneController < ApplicationController
  def update
    ...
    unless ActiveSupport::TimeZone[time_zone]
      redirect_to settings_path(section: 'time_zone'), alert: 'Unknown time zone.', status: :see_other and return
    end

    current_user.update!(settings: (current_user.settings || {}).merge('time_zone' => time_zone))
    redirect_to settings_path(section: 'time_zone'), notice: 'Time zone saved', status: :see_other
  end
end
